public class Complex {
  ...
  public Complex quotient(Complex that) {
    double imConj = that.im * -1;
    double abs = (Math.pow(that.re, 2) + Math.pow(that.im, 2));
    double _Re = ((this.re * that.re) - (this.im * imConj)) / abs;
    double _Im = ((this.re * imConj) + (that.re * this.im)) / abs;
    return new Complex(_Re, _Im);
  }
